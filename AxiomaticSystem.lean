...
- AxiomaticSystem Phase 1
  ...
  - ie. theorem example : χ := -- where 'β' is an AxiomaticSystem in which one of the axioms is of type 'χ' 
          α : χ := β -- the reference to AxiomaticSystem 'β' would automatically return the axiom of type 'χ'
- AxiomaticSystem Phase 2
  - create a function that can deduce pseudo-random theorems from axiomatic systems, generating new a AxiomaticSystem that includes the theorem as a redundant axiom 
...
- AxiomaticSystem Phase 3
  ...
  - this function will be of type Bool where true means that they are reducible and false means that they are not
- AxiomaticSystem Phase 4
  ...
  - this function will be of type AxiomaticSystem
-/
